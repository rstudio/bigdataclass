...
3. Use `sdf_bind_cols()` to combine two tables of the same row size
```{r}
sdf_bind_cols(
  spark_mtcars,
  spark_mtcars
...
4. Use `sdf_bind_rows()` to append Spark tables of the same width
```{r}
sdf_bind_rows(
  mutate(spark_mtcars, table = 1),
  mutate(spark_mtcars, table = 2)
) 
```
