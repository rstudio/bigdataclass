...
if(dir.exists("saved_model")) unlink("saved_model", recursive = TRUE)
...
1. Read the top 5 rows of the 2008 flights CSV file.  It is located under /usr/share/flights
```{r}
library(readr)
top_rows <- read.csv("/usr/share/flights/flights_2008.csv", nrows = 5)
```

2. Create a list based on the column names, and add a list item with "character" as its value.
```{r}
library(purrr)
...
3. Use `spark_read()` to "map" the file's structure and location to the Spark context
```{r}
...
4. Verify that the new variable pointer work using `tally()`
```{r}
spark_flights %>%
  tally()
```
...
2. Register the data subset as a new table called *sub_flights*
```{r}
sdf_register(cached_flights, "sub_flights")
```

3. Use `tbl_cache()` to extract the data into Spark memory
```{r}
tbl_cache(sc, "sub_flights")
```

4. Confirm new variable pointer works
```{r}
cached_flights %>%
  tally()
```
...
## `sdf` Functions
*Overview of a few `sdf_` functions*
